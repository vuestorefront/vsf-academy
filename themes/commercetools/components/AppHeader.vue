...
<script>
  ...
  import { useCart, useWishlist, wishlistGetters, useUser, cartGetters,  useFacet } from '<%= options.generate.replace.composables %>';
  ...
  export default {
    ...
    setup(props, { root }) {
      ...
      const wishlistIcon = computed(() => wishlistGetters.getTotalItems(wishlist.value) ? 'heart_fill' : 'heart');
