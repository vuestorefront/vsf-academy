  ...
  <div id="category">
    ...
    <div class="navbar section">
      ...
      <div class="navbar__main">
        ...
        <div class="navbar__counter" v-if="pagination">
        ...
        <div class="products" v-if="!loading">
          ...
          >
            <SfProductCard
              ...
              :key="searchGetters.getItemId(product)"
              :style="{ '--index': i }"
              :title="searchGetters.getItemName(product)"
              :image="
                searchGetters.getItemImages(product)[0] &&
                searchGetters.getItemImages(product)[0].url
              "
              :regular-price="
                $n(searchGetters.getItemPrice(product).regular, 'currency')
              "
              :special-price="
                searchGetters.getItemPrice(product).special &&
                $n(searchGetters.getItemPrice(product).special, 'currency')
              "
              :max-rating="5"
              :score-rating="5 || searchGetters.getAverageRating(product)"
              ...
              :link="
                localePath(
                  appendFromCategory(
                    `/p/${searchGetters.getItemId(
                      product,
                    )}/${searchGetters.getItemSlug(product)}`,
                  ),
                )
              "
          ...
          >
            <SfProductCardHorizontal
              v-e2e="'category-product-card'"
              v-for="(product, i) in items"
              :key="searchGetters.getItemSlug(product)"
              :style="{ '--index': i }"
              :title="searchGetters.getItemName(product)"
              :description="
                'desc to fulfill' || productGetters.getDescription(product)
              "
              :image="
                searchGetters.getItemImages(product)[0] &&
                searchGetters.getItemImages(product)[0].url
            ...
            >
              <template #configuration>
                <SfProperty
                  v-if="sizeAttributes[i]"
                  class="desktop-only"
                  :name="sizeAttributes[i].name"
                  :value="sizeAttributes[i].value"
                  style="margin: 0 0 1rem 0"
                />
                <SfProperty
                  v-if="colorAttributes[i]"
                  class="desktop-only"
                  :name="colorAttributes[i].name"
                  :value="`${colorAttributes[i].value.label[$i18n.locale]}`"
                />
              </template>
              <template #actions>
                <SfButton
                ...
                <SfButton
                  ...
                  class="sf-button--text desktop-only"
                  style="margin: 0 0 1rem auto; display: block"
                  @click="removeProductFromWishlist(product)"
                >
          ...
          <div
            v-show="pagination && pagination.totalPages > 1"
    ...
    <LazyHydrate when-idle>
      ...
      >
        <div class="filters desktop-only">
          <div v-for="(facet, i) in filters" :key="i">
            <SfHeading
              :level="4"
              :title="facet.label"
...
export default {
  ...
  setup(props, context) {
    ...
    const { result, search, loading } = useSearch('cat-page')
    const {
      app: {
        i18n: { locale },
      },
    ...
    const items = computed(() => searchGetters.getItems(result.value))
    const categoryTree = computed(() =>
      searchGetters.getCategoryTree(result.value),
    )
    const breadcrumbs = computed(() =>
      searchGetters.getBreadcrumbs(result.value),
    )
    const sortBy = computed(() => searchGetters.getSortOptions(result.value))
    const pagination = computed(() => searchGetters.getPagination(result.value))
    const filters = computed(() => searchGetters.getFilters(result.value))
    const colorAttributes = computed(() => [])
    const sizeAttributes = computed(() => [])

    const activeCategorySlug = computed(() => {
      return $route.path.replace('/c/', '')
    })

    const appendFromCategory = (uri) => {
      return `${uri}?fromCategory=${activeCategorySlug.value}`
    }
    ...
    onSSR(async () => {
      ...
      const facetFilters = Object.entries(filters).reduce(
        (total, [filterType, values]) => {
          const filterValues = []
      ...
      return search({
        ...
        parameters: {
          ...
          facetFilters: [
            ...facetFilters,
            `categories.${locale}.slug:${Object.values($route.params)
              .filter(Boolean)
              .join('/')}`,
          ],
        },
    ...
    onMounted(() => {
      ...
      if (!filters.value || !filters.value.length) return
      selectedFilters.value = filters.value.reduce(
        (prev, curr) => ({
          ...prev,
          [curr.id]: curr.values.filter((o) => o.isSelected).map((o) => o.id),
        }),
        {},
      )
    })
